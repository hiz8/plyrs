...
| 10 | `2026-07-18-phase10-privileged-tenant-and-ops.md` | 完了(2026-07-19 main へマージ) |
...
（→ 上記 7 件はすべて Phase 6b の Task 1・2 で消化済み）

## 12. Phase 6b 完了時の申し送り（最終レビュー 2026-07-17）

Phase 6b（コンテンツモデリングと record 編集）完了。527 tests green（6a 時点 454 → +73。
内訳: metamodel 46 / db 13 / ui 22 / sync-protocol 15 / sync-client 62 / admin 85 / api 284）。
typecheck / lint（警告 0）/ format:check clean。最終ブランチレビュー（最上位モデル）は
「マージ可」— ただし下記 Important 2 件の申し送り記録を条件とした（2026-07-17 ユーザー裁定:
申し送りして 6b はマージ、解消は Phase 7）。

**裁定（2026-07-17、6 点）:** richtext = 読み取り専用プレースホルダー（値は input 透過保持、
changedFields に載らない）/ relation = 同期コレクション駆動 picker（one=Select・many=
CheckboxGroup、未知型は候補 0 件に退化）/ content_type ビルダー = 新規 + 既存編集（key は
id に対し不変、version はサーバー管理）/ 同期接続 = /t/$tenantSlug レイアウトの寿命（mount
start・unmount/切替/logout stop・online と再接続ボタンで start 再呼び出し）/ publish・status
UI = コアが record-editor:toolbar（core.publish / core.status）と :panel（core.publication）へ
スロット自己登録 / スカラー競合 UI = 最小（SyncRejectedError のエラーバナーのみ）。

**§8 の sync-client 配線契約 5 点は全消化**（apps/admin/src/lib/sync.ts に集約。
onContentTypes→registry.sync / onReady→markReady / onStoreChange→applyStoreChange /
onReset→reset / `let registry!` 前方参照。markReady は変異検証付きの回帰テストあり —
onReady 配線を外すと sync.test.ts が FAIL することを実測確認済み）。

**Phase 7 で必ず解消する 2 件（最終レビュー Important・計画コード由来の編集 UX 設計負債）:**

1. **一時切断でエディタのフォームが未保存入力ごと消える**: record 系 3 ルートの
   `status !== "ready"` ゲートが、再接続中（connecting/syncing/closed）にフォームごと
   アンマウントし、復帰後は同期値で再マウントする。オフライン編集も不可（アウトボックス
   温存の設計と不整合）。修正案: 初回 ready までのみゲートし、以降は接続バナー表示で
   フォームを維持する。
2. **マルチ編集者時、触っていないフィールドで他者の編集を静かに巻き戻す**: RecordForm の
   フォーム値はマウント時スナップショットで、保存時に全フィールドを draft から書き戻す。
   編集開始後に他者の変更が WS で届いたフィールドは、保存時に古い値へ上書きされ、
   baseFieldVersions に相手の新 version が入るため LWW がクリーン上書きとして受理する
   （§10.3 のフィールド粒度マージを form 経由の編集が部分的に無効化）。修正案: マウント時
   draft を基準に「ユーザーが実際に変えたキーだけ」を書き、他は現在行の値を維持する。
   §8 の「連続編集の自己競合」の多人数版として、Phase 7 の競合 UI と同時に解く。

**技術的な確定事項・落とし穴:**

- **@tanstack/react-db は不採用**: 0.1.x は @tanstack/db 0.6.16 を固定依存に持ち、catalog の
  0.6.14 と二重インスタンス化するため。React からのコレクション購読は
  apps/admin/src/lib/use-collection.ts（subscribeChanges + useState）の自前フックで行う。
- **RAC Select のフルページ jsdom テストは userEvent + Popover 経由が不安定**（CPU スピンを
  実測）。publish-slots.test.tsx の様式 = RAC の隠しネイティブ select への fireEvent.change
  （useSelectState.setValue → onSelectionChange と同一コードパスであることをレビューで実証
  済み）。Select 単体の open/click 検証は packages/ui/src/select.test.tsx が担う。
- **admin テストの負荷フレーク対策**: vitest.setup.ts の `configure({ asyncUtilTimeout: 5_000 })`
  + vitest.config.ts の `testTimeout: 15_000`（14 ファイル並列時の findBy 1 秒切れ対策）。
  症状対処であり、テストが増えて再発したら並列度（poolOptions / fileParallelism）で根治する。
- **CheckboxGroup（packages/ui）は RAC 不使用**の `div role="group"` 実装（RAC CheckboxGroup
  は group state が isSelected prop を無視するため構造的に衝突 — 実測で確認）。errorMessage
  は useId ベースの aria-describedby で配線済み。
- **公開状態の read API**: `GET /v1/t/:tenantId/records/:recordId/publication` →
  `{ published: false } | { published: true, publishedAt, publishedBy, sourceVersion }`
  （record の存在は見ない = §7 の真実源）。admin 側の PublicationState は構造ミラー。
- **status 変更は同期経路（LWW）・publish/unpublish は HTTP** の二経路。status のみの変更は
  wire 上 `changedFields: [] + status`。StatusControl は publication クエリ未解決の間 Select を
  disabled にする（§7 archive 警告のレース対策 — 手動解決 Promise の回帰テストあり）。
- token-manager に `getToken(tenantId, { forceRefresh: true })` を追加（WS 4001 と admin-api の
  401 1 回リトライが使用。forceRefresh は飛行中リクエストに相乗りする — 失敗直後の同一
  トークン再取得になりうるトレードオフは許容済み）。
- relation picker の draft キーは `type + "" + id`（U+001F はエスケープ表記で書く —
  生バイト混入をレビューで検出・修正した経緯あり）。
- RecordForm の form 要素には `noValidate` が必須（RAC の validationBehavior 既定 native が
  ブラウザ制約検証で submit を止め、zod 側の検証に到達しない）。

**Minor 積み残し（Phase 7 以降の掃除候補。詳細は 6b ブランチの各タスクレビュー）:**
publication クエリ失敗時に StatusControl が恒久 disabled / パネルが「読み込み中…」のまま
（isError 表示なし）/ STATUS_LABELS が status-control.tsx と records 一覧ルートに重複 /
useRelationCandidates はマウント後に新規作成された許可型のコレクションを再マウントまで
購読しない / content-type-form の FieldDraftCard が key={index} / record-form.tsx の境界 cast
3 箇所に理由コメント欠落 / publication クエリが 3 スロットコンポーネントで重複（React Query
がデデュープするため実害なし・フック化余地）。

**手動確認が必要な項目（6b では実機確認を行っていない）:**

1. `pnpm --filter @plyrs/admin dev` で実 WS 同期: 2 タブで同一 record を開き、片方の編集・
   status 変更・削除がもう片方へ反映されること。
2. ビルダーで型作成 → record 作成 → publish → `/public/v1/:slug/records/:type` で公開 API
   から見えること（投影 consumer 経由、数秒の結果整合）。
3. WS 再接続: dev サーバー再起動後の「再接続」ボタン復帰、オフライン → online イベントでの
   自動再開。
4. トークン失効（15 分放置）後の 4001 → forceRefresh → 再接続。
5. 6a から持ち越しの手動確認 4 項目（§11）は依然未実施。

## 13. Phase 7 完了時の申し送り（最終レビュー 2026-07-17）

Phase 7（リッチテキスト）完了。578 tests green（6b 時点 527 → +51。内訳: metamodel 56 /
db 13 / ui 33 / sync-protocol 15 / sync-client 62 / admin 112 / api 287）。typecheck /
lint（警告 0）/ format:check clean。最終ブランチレビュー（最上位モデル）は Important 2 件の
対処（1 件コード修正・1 件は本申し送りへの記載）を経て「マージ可」。

**裁定（2026-07-17、6 点）:** 語彙 = StarterKit v3 標準（見出し1-3・リスト・太字/斜体/
コード・コードブロック・引用・外部リンク・下線）+ 汎用 record mention ノード / 保存モデル =
明示保存ボタン（オートセーブ・エンジン改修なし）/ 競合 UI = 二択ダイアログ（sync-protocol
拡張なし・手元 store 突き合わせ）/ §12 必須① = 初回 ready までゲート + 以降は接続バナーで
フォーム維持 / §12 必須② = マウント時 draft 基準の dirty キーのみ書き戻し（RecordForm 全体）
/ AST 検証 = 語彙非依存の構造検証（ノード型名の語彙は見ない）。

**§12 必須 2 件は解消。** ①: `TenantSync.getHasSynced()` + ConnectionBanner（record 系
3 ルート共通。切断・再同期中も未保存入力ごとフォームを維持）。②: `fromDraftValues(…,
initialDraft)` の dirty-only merge + 保存成功/サーバー版採用時の基準前進。ワイヤレベル回帰は
apps/admin/src/richtext-flow.test.tsx（push の changedFields / baseFieldVersions / envelope、
競合二択の両分岐、§8 自己競合ガード、切断時フォーム維持）。

**最重要（最終レビュー Important #1）: 旧形式（非構造）richtext doc を持つ既存レコード。**
Phase 7 の構造検証（doc = `{type: string, ...}` の再帰ツリー）により、旧形式 doc を持つ
レコードは validate-on-write で全体が validation_failed になる — untouched キーにもサーバー
現在値が合成され全量検証されるため、**無関係なフィールドの編集も保存不能**。さらに admin は
旧形式値を空エディタとして表示するため、そのまま本文を入力して保存すると旧内容を見ずに
上書きし得る。回復手順: 本文を一度正しい形で保存すれば解消（遅延適合）。実データが開発用に
限られる現状で許容した。**実テナント運用開始前に既存 records の doc 形式を点検すること。**

**語彙進化の制約（Phase 8 で最初に効く）:** 構造検証は語彙を見ないため metamodel は将来
ノードを素通しするが、**Tiptap は未知ノードを含む doc を安全に開けない**（エディタ上で
欠落し、編集・保存すると失われる）。Phase 8 の画像ノードは metamodel ではなく admin/ui の
語彙デプロイと同期して投入すること（サーバー先行は安全、クライアント遅れは危険）。

**公開配信の責務（Phase 8 以降）:** 公開 read 側で AST を HTML 化するレンダラを作る場合、
link mark の href サニタイズ（javascript: 等）はそのレンダラの責務。admin 内は
openOnClick:false + Tiptap 既定の URI 検証で防護済み。

**技術的な確定事項・落とし穴:**

- Tiptap は catalog `^3.28.0` の 6 パッケージ（core / extension-mention / pm / react /
  starter-kit / suggestion）。依存は packages/ui のみ（admin は @plyrs/ui 経由）。
- ProseMirror の jsdom シム（Range 測定 API / scrollIntoView / elementFromPoint）は ui と
  admin の vitest.setup.ts に同一ブロック。**パッケージ全域に効く**（将来 RAC の実座標依存
  テストと衝突したら再訪）。packages/ui/src/tiptap-jsdom.test.ts がカナリア。
- **エディタのテスト様式 = コマンド駆動**（jsdom はタイピングを再現できない）。
  `onEditorReady` で editor を捕まえ `editor.commands` / `chain()` で操作（6b の隠し
  ネイティブ select と同型の判断 — コマンドはキー入力と同じ transaction dispatch を通る）。
  ルートテストはツールバークリック（見出し1 等）で doc を変える。
- mention ノード契約: type `"recordMention"`、attrs `{recordType, recordId, label}`（label は
  挿入時点スナップショット。relations には投影しない）。metamodel `RECORD_MENTION_NODE_TYPE`
  ⇄ ui `RECORD_MENTION_NODE_NAME` の一致は apps/admin/src/lib/mention-contract.test.ts が固定
  （ノード名のみ — **attrs キーまで固定するクロスパッケージテストを Phase 8 冒頭で追加推奨**）。
- mention の renderHTML は `data-type` + `data-record-*` を必ず出す（コピペ往復 = HTML
  serialize→parse の契約。欠くと mention がプレーンテキストへ静かに退化する — 回帰テストあり）。
- origin='body' は write-record.ts が毎書き込みで全消し→再挿入。同期（loadRelationRefs）・
  公開 read（include.ts）は origin='field' フィルタ済みで汚染なし。publish スナップショットは
  origin 込みで凍結。delete は blanket delete が body 行も掃除。
- conflict ack の突き合わせ相手 = 手元 store の最新 record（サーバーは他者の change を ack
  より先に配信する）。§8 自己競合ガードは render 時 diff — ダイアログは構造的にフラッシュ
  しない（change 先行配信なら初回レンダーから realConflictKeys が空）。
- adoptServer はサーバー側の本文削除を `EMPTY_RICH_TEXT_DRAFT`（明示空 envelope）へ写す
  （undefined のままだと RichTextEditor が「初期状態」として無視し自分の版が残る — 最終
  レビューで検出・修正済み）。
- `pnpm --filter X test -- <pattern>` は pnpm が `--` を転送するためフィルタにならない。
  `pnpm --filter X exec vitest run <pattern>` を使う。
- verify-commit-message.sh は .git/hooks に実体がなく、50 字制限はフックでは強制されない
  （本フェーズは手動検査で維持。フックの再セットアップは Phase 10 の運用整備候補）。

**Minor 積み残し（Phase 8 以降の掃除候補・最終レビューでトリアージ済み）:**
body-relations の dedup キーコメント不一致（uuid 固定長で実害なし）/ AST 深さ上限なし
（DO は transactionSync + try/catch で internal_error に収まり部分書き込みは無い — clean な
validation_failed 化の余地）/ ConflictDialog のフォーカス管理・mention リストの
aria-activedescendant（a11y バックログ）/ サーバーリセット + 編集中の「レコードが見つかり
ません」フォーム置換（既存挙動）/ 値同期 effect の JSON.stringify 2 回/レンダー（大 doc の
最適化余地）/ isEmptyRichTextValue が空 text ノード単体の段落を空と見なさない（現行
プロデューサで到達不能）/ write-record の field/body 挿入ループ重複。

**手動確認が必要な項目（Phase 7 では実機確認を行っていない）:**

1. 実ブラウザでのタイピング・IME 日本語入力（composition）・キャレット移動・undo/redo。
2. mention の実操作: `@` ドロップダウンの表示位置・クリック確定・別タブで record 追加 →
   候補のライブ更新。
3. 本文競合の実 2 タブ再現: 片方で本文編集 → 他方で編集 → 保存 → 二択ダイアログの両選択肢
   （サーバー版採用 / 自分の版で上書き）。
4. 切断バナー: dev サーバー再起動でフォーム維持・未保存入力保持。オフライン中の保存が
   アウトボックス経由で再接続後に届くこと。
5. mention 入り本文 → 保存 → publish → 公開 API 応答の fields に body 由来関係が漏れない
   こと・DO の relations に origin='body' 行が実在すること。
6. リンク: openOnClick 無効の確認・リンク済みテキストの再編集 UX。
7. 6a/6b からの手動確認バックログ（§11・§12）は依然未実施。

## 14. Phase 8 完了時の申し送り（最終レビュー 2026-07-18）

Phase 8（アセット）完了。653 tests green（Phase 7 時点 578 → +75。内訳: metamodel 65 /
db 14 / ui 38 / sync-protocol 15 / sync-client 62 / admin 128 / api 331）。typecheck /
lint（警告 0）/ format:check clean。最終ブランチレビュー（最上位モデル）は Important 1 件
（下記 r2_key ガード）の修正（2a4689d）後に「マージ可」。

**裁定（2026-07-17、7 点・詳細は計画の裁定事項）:** アップロード = Worker 経由 PUT
（presigned 不採用）/ asset = システム型 + 改変防御（filename/content_type/size/r2_key/
width/height はシステム管理、alt/caption のみユーザー編集可）/ 公開配信 = 投影ゲート付き
`GET /public/v1/:tenantSlug/assets/:assetId`（DO 非経由・Cache API 前段）/
snapshotEmbed "value" = allowedTypes がちょうど `["asset"]` の関係限定 / 本文画像 =
assetImage ノード（attrs は mention と同型）を admin/ui と同時投入（§13 の語彙進化制約の
実装形: ノード常時登録・挿入ボタンのみゲート）/ orphan = UI フィルタ + 使用箇所表示 /
publish は参照 asset（field + body 由来）を同一トランザクションでカスケード（unpublish は
非カスケード）。

**最終レビューで検出・修正済み（Important）: r2_key のテナント帰属未検証。**
assetGuardHook は content_type の `source === 'system'` でのみ働くため、Phase 8 以前に
user 型 `key='asset'` を作っていたレガシーテナントでは ensureAssetContentType が登録を
スキップする一方、asset 系ルートは `record.type === 'asset'` だけで動作し、任意 r2_key に
よる**他テナント R2 オブジェクトの読み取り・削除**が成立し得た。修正:
`assetKeyBelongsToTenant`（apps/api/src/assets/ownership.ts、`${tenantId}/` 区切り込み
前方一致）を 3 経路（認証プレビュー / DELETE の R2 掃除 / 公開配信 — 公開側は slug 解決後
tenantId）の ASSETS.get/delete 直前に配置。レガシー状態を runInDurableObject の直接 SQL で
再現した実 HTTP 統合テスト（test/asset-ownership.test.ts）で 3 経路とも固定。
**教訓: asset 系の新経路を足すときは必ずこのガードを併設すること**（assetGuardHook は
システム型前提で万能ではない）。

**運用上の申し送り（Phase 9/10 で効く）:**

1. レガシー user 型 'asset' テナントの**手動移行手順は未定義**（ガードで悪用は封じたが、
   当該テナントはアセット機能が半動作のまま）。実テナント運用開始前に §13 の旧形式
   richtext 点検と同時に点検すること。
2. **同期 push 経由の asset record 削除は R2 バイナリを掃除しない**（Worker の DELETE
   経路のみ掃除）。孤児バイナリの GC は Phase 10（計画どおり）。
3. **凍結 embed の url はオリジン相対 + tenantSlug 埋め込み**。公開 API 利用者はオリジンを
   自前で前置する。テナント slug 変更を実装する際は既公開記事の再 publish（または埋め込み
   URL の恒久 ID 化）が必要。
4. **asset の手動 unpublish は公開中記事の凍結 URL を 404 にする**（unpublish 非カスケード
   の帰結）。Phase 9 で unpublish 前の usage 警告 UI を検討（usage RPC は実装済みで流用可）。
5. 公開アセットは Cache API TTL（30 秒）の間、unpublish/削除後も配信されうる（§10 の
   キャッシュ裁定と同型のトレードオフ）。
6. 凍結 embed の alt/寸法は **publish 時点の record 現在値**（buildAssetEmbed は編集の
   真実源から読む設計判断）。asset の alt を編集しても記事を再 publish するまで embed は
   古いまま — 公開 API ドキュメント整備時に明記すること。
7. admin の objectURL は revoke されない・失敗した resolveUrl が恒久メモ化される
   （キャッシュ無効化は将来チケット。401 リトライ後の画像復帰が現 UI で効かない点を含む）。

**技術的な確定事項・落とし穴:**

- ensureAssetContentType は **zod 正規化後の定義比較で冪等**（正規化前比較だと hibernation
  復帰ごとに version が無限に進む実バグを TDD で検出・修正済み）。key 衝突時は throw せず
  スキップ（テナント全損回避）。
- migration 0004 は `ALTER TABLE ADD COLUMN` 1 文のみ（§5 の records 再ビルド禁止規約を
  遵守。旧 snapshot は `embed?: AssetEmbed | null` の optional で後方互換）。
- publishRecord の署名は `(tenantId, tenantSlug, recordId, auth)`（凍結 URL の slug は
  publish 時に実 D1 から解決）。
- レガシーテナント状態のテスト再現様式 = runInDurableObject で content_types 行を直接
  SQL 書き換え（test/asset-ownership.test.ts 参照。id を別 UUID にして ensure の自己修復で
  偽陽性化しないようにする）。

**Minor 積み残し（Phase 9+ の掃除候補・最終レビューでトリアージ済み）:**
image-size の u32be 符号（壊れヘッダの負寸法を null に畳む 1 行）/ upload の content_type
切り詰めが filename 定数を流用 / body 由来カスケードの直接テスト / many + embed の
ordinal テスト / ui の nodeView resolver に `.catch` なし（パッケージ公開 API として
unhandled rejection の余地）/ a11y 束（alertdialog フォーカストラップ・upload エラーの
role=alert — §13 の ConflictDialog と同束で 1 タスク化推奨）/ 削除ダイアログの usage
取得中に確定ボタンが押せる / 選択ダイアログが選択済みアセットを明示しない /
record-form ⇄ asset-picker の labelForRecord モジュール循環(lib へ抽出) /
ensure skip 分岐テストの実コンストラクタ経路化（evictDurableObject + ping）/
upload で R2 put が DO の役割検証より先（viewer でも put→best-effort delete が走る）。

**手動確認が必要な項目（Phase 8 では実機確認を行っていない）:**

1. 実ブラウザでのアップロード → 一覧サムネイル → プレビュー配信。
2. record の publish → 公開 API の embed 返却 → `/public/v1/:slug/assets/:id` の実配信
   （カスケード publish の実挙動含む）。
...
## 16. Phase 10 完了時の申し送り(最終レビュー 2026-07-19)

Phase 10(特権テナント + 運用)完了。764 unit tests green(Phase 9 時点 707 → +57。内訳: metamodel 65 /
db 16 / ui 38 / sync-protocol 15 / sync-client 62 / admin 144 / api 424 — 最終修正込みで api 426)+
Playwright E2E 3 本(特権コンソール / コアジャーニー / 2 タブ WS 同期)。typecheck / lint(警告 0)/
format:check clean。最終ブランチレビュー(最上位モデル)は Important 2 件の修正(44e1f90)後に「マージ可」。

**裁定(2026-07-18〜19、11 点):** 特権認証 = パスワード + TOTP 必須(RFC 6238 自前実装・passkey は
将来枠)/ 特権アカウント = super_admins 別テーブル完全分離 + __Host-plyrs_super_session 別 cookie /
監査 = 中央 D1 audit_logs(append-only・削除 API なし)/ ブロックリスト = (userId, tenantId) KV 二層
(TTL 1200s)/ E2E = スモーク 3 本 / 運用機能 = 全部入り(健全性・DLQ・トリガー・§6 束)/ §15 Minor
掃除 = 実害系 3 件のみ / デプロイ = wrangler env 分割 + GH Actions(preview 自動・production 手動)/
**テナント作成は super 専用化**(POST /v1/tenants 廃止・admin 作成 UI 撤去 — Phase 3 の暫定開放を閉鎖)/
E2E の公開 API 到達は dev 限定転送(DEV_PROXY_PUBLIC=1。6a 裁定は本番で維持)/ 壊れた
module_registry.permissions 行は MODULE_REGISTRY からマニフェスト再導出(fail-open 禁止)。

**最終レビューで検出・修正済み(Important 2 件・44e1f90):**
1. **テナント削除カスケードの再実行不能 + 監査空白**: tenants 行を最初に消す計画順序だと、後段
   (DO wipe / 投影 / R2)の失敗後に再実行 DELETE が 404 になり残骸が API から掃除不能だった。
   tenants 行削除を最終段へ移し(KV slug 削除はその後 — 先に消すと正キャッシュ 300s が再形成される)、
   監査を phase:start(カスケード開始前)/ phase:complete の 2 行に。失敗注入つき再実行テストで固定。
2. **削除済みテナントの旧 JWT 復活ウィンドウ**: tenantGate は control-plane を照会しないため、削除後も
   有効期限内(≤15 分)の JWT で wipe 済み DO を空スキーマで復活させ書き込めた。カスケード冒頭で
   全旧メンバーに blockMembership(TTL 1200s > JWT 900s)を張って封鎖。**教訓: 「control-plane 行の
   削除がゲートを止める」は誤り — ゲートは JWT + KV blocklist のみを見る。テナント単位の即時失効は
   必ず blockMembership を張ること。**

**タスクレビューで検出・修正済みの主要事象(いずれも計画の参照コード由来):**
- TOTP counter 前進を CAS 化(`WHERE totp_last_counter < counter` + 変更行数判定。並行 2 リクエスト
  実発火の回帰テストで [200,401] を固定 — CAS なしだと [200,200] になることを実測)。
- bootstrap を単文 `INSERT ... WHERE NOT EXISTS` へ(異なる email の同時 bootstrap 二重成功を封鎖。
  並行回帰テストあり)。
- モジュール alarm の throw 隔離は transactionSync の**外側**で catch(内側 catch だと部分書き込みが
  commit され alarm 再武装も失われる — runDurableObjectAlarm 経由の統合テストでロールバックを固定)。
- 孤児 R2 削除ルートの assetKeyBelongsToTenant は引数順 (r2Key, tenantId)(計画スニペットは逆で
  ガードが反転していた — §14 の教訓を再演。実装者が検出)。

**運用上の申し送り(実運用開始前に効く):**
1. **実デプロイは未実施**。docs/deploy.md の §1(リソース作成・ID 転記)完了までは main push の
   preview デプロイは失敗する(想定内)。**デプロイ後は直ちに /super-auth/bootstrap を実行すること**
   (super_admins が空の間は到達可能な誰でも初期化できる — レート制限のみ)。TOTP 紛失時の復旧 SQL は
   deploy.md §4。
2. admin の実 `vite build` 完走と CI の e2e ジョブ初回完走は実 CI で未証明(sandbox では prerender が
   Request.cf timeout で落ちるが routeTree 生成・dist/server/wrangler.json 出力までは検証済み)。
   マージ後最初の CI 実行を必ず確認すること。
3. **E2E はローカル永続化(apps/{admin,api}/.wrangler)を毎回削除する**(bootstrap 空前提の再現)。
   開発中のローカル D1 データは消える。専用 persistState 分離は将来課題。E2E の retries: 1 は
   bootstrap/slug が run 内固定のため実質無効(リトライ吸収にするなら attempt ごと一意化)。
4. ローカル .dev.vars の JWT_SECRET は 32 字未満だと全 API が 500(fail-closed)。
5. DLQ は D1 退避(dead_letters)+ 特権コンソールからリプレイ/破棄。保持期限なし(無期限蓄積)。
   削除前に in-flight だった投影ジョブがカスケード後に着地して孤児行を残す余地は許容
   (§9 の墓標 GC 見送りと同じ箱)。
6. record エディタは他クライアントの保存をライブ反映しない(マウント時バインド + dirty-only 保存の
   設計帰結)。「他所で更新された」表示もない — UX 改善候補。E2E の 2 タブ検証は records 一覧の
   反応性で実施。
7. 見送り(裁定済み): 墓標周期 GC / 投影 QPS 監視 / getPublishedPage バイト予算 / 公開 read
   レート制限 / passkey 格上げ / テナント slug 変更 / otpauth QR 表示 / dead_letters 保持期限。

**技術的な確定事項・落とし穴:**
- api の Env は apps/api/env.d.ts 手書き(cf-typegen なし)。admin は cf-typegen に
  `--strict-vars=false`(vars がリテラル型に潰れて比較が TS2367 になるため)。
- AUTH_LIMITER は miniflare が実シミュレートする(グローバルバケット共有)— auth 系テストは
  test/rate-limit-helper.ts の fake を env に必ず spread する。
- キュー名規約: `plyrs-<用途>[-環境]` + DLQ は末尾 `-dlq`(環境サフィックスは -dlq の前)。
  queue() の判別は endsWith("-dlq") 優先 → startsWith("plyrs-modules")。
- super 系テストの共有 D1 規律: super_admins/super_sessions は test/super-login.ts の
  resetSuperAdmins で afterEach 掃除。テナント作成は test/create-tenant.ts の direct insert
  (HTTP 経路は super 専用化で廃止済み)。
- root に wrangler devDependency あり(pnpm exec wrangler の root 解決に必須 — deploy.yml が依存)。
- Playwright の globalSetup は webServer 起動を待たない — E2E のセットアップは package.json の
  test スクリプト前段(node --experimental-strip-types)で実行。auxiliaryWorkers の dev では D1
  migration が自動適用されない — global-setup が `wrangler d1 migrations apply --local` を明示実行。

**Minor 積み残し(次フェーズの掃除候補・最終レビューで全件申し送り可と裁定):**
hasValidPermissionsShape が配列を通す(!Array.isArray で全閉)/ runModuleAlarmHandler の未使用
moduleId / statusFor の「`:` 含み → 409」の広さ / turnstileToken 空文字の 403 化(min(1) で 400 早期化)/
banUserEverywhere の逐次 disconnect / super テナント作成 slug TOCTOU の素 500(unique 索引が防衛)/
users 検索 LIKE の % _ 未エスケープ・membershipCount=0 未テスト / DLQ 再投入・再配布ボタンの
isPending disable なし / super 4 ページの styles 複製 / env サフィックス付き modules キュー実配送の
直接テストなし / index.ts queue() doc コメントのドリフト / super/route.tsx「401 以外再送出」の
直接テストなし / ログアウトクリック遷移テストなし / rename・unban・ownerEmail 省略 body の専用
テストなし / control-plane.test.ts の到達しないフォールバック / /super-auth/me の削除済み admin
email null。

**手動確認が必要な項目(Phase 10 で実機確認していないもの):**
1. 実 CI での admin build / e2e ジョブの完走(申し送り 2)。
2. 実デプロイ(preview 環境)での特権コンソール一式・DLQ リプレイ・再投影の実挙動。
3. 実 Turnstile(signup/login の AUTH_TURNSTILE 有効化時のウィジェット表示)。
4. §11〜§15 の手動確認バックログのうち E2E 3 本が吸収しなかった項目(IME・richtext 実タイピング・
   アセットアップロード実機・モジュール予約フロー実機)は依然未実施。
