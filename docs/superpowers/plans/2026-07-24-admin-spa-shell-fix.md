...
- main へローカルマージ → push → Deploy workflow 完走 → 実機 curl。
  素の curl(Accept: */*)は resolveServerRoute が `"ssr"` と判定するため、200 でも
  防御 B(SSR フォールバック)の成果しか観測できない。検証は 2 項目に分けて行う:
  - (a) `curl -H 'Accept: text/html' <url>/super-login`(`/super` `/tenants` も同様)が
    200 で、シェル HTML(シリアライズ済みエラーなし・pending match)であること
    → 防御 A(シェル配信)の確認。
  - (b) 素の curl(Accept 指定なし)で `/super-login` `/super` `/tenants` が 200 で
    あること → 防御 B(SSR フォールバック)の確認。
